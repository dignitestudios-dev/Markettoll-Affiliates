      ...
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className="relative shrink-0">
          {hasCustomImage ? (
            ...
            <div className="w-11 h-11 rounded-full bg-[#0098EA] text-white flex items-center justify-center font-bold text-lg shadow-sm shrink-0 uppercase">
              {initial}
            </div>
          )}
          {isUnread && (
        ...
        <div className="min-w-0 flex-1 pr-2">
          ...
          >
            {name} {(item?.order_id || item?.orderId) ? `#${item.order_id || item.orderId}` : name}
          </h4>
          <p
