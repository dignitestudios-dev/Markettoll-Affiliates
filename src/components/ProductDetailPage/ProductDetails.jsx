...
const ProductDetails = () => {
  ...
  const isBoosted =
    ...
    product?.boostPlan?.expiresAt &&
    new Date(product?.boostPlan?.expiresAt) > new Date();
  const createdAt = product?.boostPlan?.purchasedAt;
  const expiresAt = product?.boostPlan?.expiresAt;
            ...
            <div className="w-full flex flex-col items-start gap-5 lg:max-w-1/2">
              <div className="w-full flex flex-col lg:flex-row items-start lg:items-center justify-between">
                <div className="flex flex-wrap items-center gap-5">
                  ...
                  {isBoosted && (
                    <div className="inline-flex items-center gap-3 bg-[#E0F7FF] text-[#00AAD5] rounded-full px-4 py-2 shadow-sm">
                      ...
                      <span className="bg-[#00AAD5] text-white px-2 py-0.5 rounded-full text-xs font-medium">
                        {diffDays} {diffDays === 1 ? "day" : "days"} left
                      </span>

                      <span className="text-[#5C5C5C] text-[10px] italic">
                        Expires{" "}
                        {new Date(
                          product.boostPlan.expiresAt
                        ).toLocaleDateString()}
                ...
                <div className="flex items-center justify-end gap-4">
                  ...
                  {product?.seller === userProfile?._id && (
                    <>
                      ...
                      {openDropdown && (
                        <div className="w-[151px]  bg-white border absolute top-32 rounded-xl right-14 flex flex-col items-start justify-center p-5 gap-1">
                          <Link
