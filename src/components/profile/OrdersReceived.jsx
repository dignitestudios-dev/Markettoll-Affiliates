...
const OrdersReceived = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/users/seller-orders?page=${page}&limit=${limit}`,
        {
  ...
  const handleNavigate = (order) => {
    navigate(`/order-history/order-received-details/${order._id}`, {
      state: {
        data: order,
        type: "orders-received-current",
      },
    });
  };
    ...
    <div className="w-full">
      ...
      <div className="w-full flex flex-col gap-4 mt-6 min-h-[40vh]">
        {filteredOrders.length > 0 ? (
          ...
          <h2 className="blue-text text-xl font-bold">No Orders Found</h2>
        )}
      </div>
    </div>
  );
